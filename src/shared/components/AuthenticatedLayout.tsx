...
export type AppScreen = 'home' | 'overview' | 'records' | 'sync';
...
export function AuthenticatedLayout({ children }: PropsWithChildren) {
  ...
  const navigateTo = (screen: AppScreen) => {
    if (screen === 'records' && currentScreen === 'records' && isFillingRecord) {
      setRecordsResetSignal((current) => current + 1);
      closeMenu();
      return;
    }
  ...
  return (
    ...
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      ...
      <Modal animationType="none" onRequestClose={closeMenu} statusBarTranslucent={Platform.OS === 'android'} transparent visible={isMenuOpen}>
          ...
          >
            <SafeAreaView className="flex-1" edges={['top', 'bottom']}>
              <View className="flex-row items-center justify-between px-5 pb-6 pt-4">
                <Image className="h-12 w-32 rounded-xl" resizeMode="contain" source={require('../../../assets/deep/logo.jpg')} />
                <Pressable className="h-11 w-11 items-center justify-center rounded-2xl bg-zinc-100" onPress={closeMenu}>
                  <MenuIcon dark />
                </Pressable>
              </View>
              ...
              <View className="mx-5 flex-row items-center rounded-3xl bg-zinc-100 p-4">
                ...
                <View className="ml-3 flex-1">
                  <Text className="text-base font-semibold text-zinc-950" numberOfLines={1} ellipsizeMode="tail">
                    {session?.agent.nome}
                  </Text>
                  <Text className="mt-0.5 text-sm text-primary-600" numberOfLines={1} ellipsizeMode="tail">
                    {session?.agent.tipo_agente.nome}
                  </Text>
                </View>
              </View>
              ...
              <View className="mt-6 flex-1 px-3">
                <MenuItem active={currentScreen === 'home'} label="Inicio" onPress={() => navigateTo('home')} symbol="I" />
                <MenuItem active={currentScreen === 'records' && !isFillingRecord} label="Preenchimentos" onPress={() => navigateTo('records')} symbol="P" />
                <MenuItem active={currentScreen === 'sync'} label="Sincronização" onPress={() => navigateTo('sync')} symbol="S" />
                <MenuItem active={currentScreen === 'overview'} label="Visão Geral" onPress={() => navigateTo('overview')} symbol="V" />
              </View>
